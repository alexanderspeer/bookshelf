...
export function applyRPGUICursorFixes() {
  ...
  // Preload all cursor images immediately to prevent loading delay
  const cursorImages = [
    'default.png',
    'point.png',
    'select.png',
  ...
  cursorImages.forEach(cursorFile => {
    const img = new Image();
    img.src = `${cursorBasePath}/${cursorFile}`;
  });
  
  // Apply default cursor to body immediately to force loading
  if (document.body) {
    document.body.style.cursor = `url('${cursorBasePath}/default.png'), auto`;
  }
  
  // Also create a hidden element that uses all cursors to force browser to load them
  const cursorPreloader = document.createElement('div');
  cursorPreloader.style.position = 'absolute';
  cursorPreloader.style.left = '-9999px';
  cursorPreloader.style.top = '-9999px';
  cursorPreloader.style.width = '1px';
  cursorPreloader.style.height = '1px';
  cursorPreloader.style.overflow = 'hidden';
  cursorPreloader.style.pointerEvents = 'none';
  cursorPreloader.innerHTML = `
    <div style="cursor: url('${cursorBasePath}/default.png'), auto;"></div>
    <div style="cursor: url('${cursorBasePath}/point.png') 10 0, auto;"></div>
    <div style="cursor: url('${cursorBasePath}/select.png') 10 0, auto;"></div>
    <div style="cursor: url('${cursorBasePath}/grab-open.png') 10 0, auto;"></div>
    <div style="cursor: url('${cursorBasePath}/grab-close.png') 10 0, auto;"></div>
  `;
  if (document.body) {
    document.body.appendChild(cursorPreloader);
  }
